...
func init() {
	InitializeDB()
	err := db.AutoMigrate(&gorm_gen.User{})
	if err != nil {
		fmt.Printf("Error: AutoMigrate(&gorm_gen.User{}) fail: %s", err)
	}
}
...
func Test_userQuery(t *testing.T) {
	...
	err = _do.Create(&gorm_gen.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.Save(&gorm_gen.User{})
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}

	err = _do.CreateInBatches([]*gorm_gen.User{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <users> fail:", err)
	}
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*gorm_gen.User{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
	...
	_, err = _do.ScanByPage(&gorm_gen.User{}, 0, 1)
	if err != nil {
